 ...
 * to the cluster:
 *
 * Agent-agnostic, will be used for other midonet nodes.
 * Support for schemas and node templates in zk.
 * Backwards compatible: will use java properties and the old ini file.
 * The data store is zookeeper.
 * Layer config key resolution: old file -> per-host zk conf ->
 *    zk host template -> defaults -> schema
 *
