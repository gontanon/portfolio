 ...
 *
 * This patch introduces a data structure that is meant
 * to replace the current InvalidationTrie used for flow
 * invalidation upon routing table changes.
 * 
 * The rationale for this change large memory usage under
 * adverse secnarios, such as a having a huge number of dst
 * ip addresses show up in a virtual router in a short period
 * of time.
 * 
 * The new implementation uses a 256-ary radix tree, stores
 * nothing else than ints and uses object pools to prevent
 * trie node allocation when possible.
 * 
 * Includes UTs and benchmarks.
 * 
