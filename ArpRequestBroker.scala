/* https://github.com/midonet/midonet/commit/d1ca56539e874bdb39fe2833d65e418313e6f094
 * 
 * Implement a new ARP table
 *
 * Substitute the pre-existing ARP table with a new implementation that
 * follows these principles:
 * 
 *   * Single threaded. Each simulation thread whould have its own instance
 *     and drive the ARP table in the PacketWorkFlow process() method.
 *   * No coordination for ARP requests. Thus, agents don't write null entries
 *     to the ZK replicated map anymore. Expiry is still handled by the
 *     agent that wrote the entry.
 *   * ARP requests intervals are randomized to prevent multiple ARP requests,
 *     due to lack of coordination, to be sent at the same time.
 * 
